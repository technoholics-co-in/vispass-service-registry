...
/**
 * Known callers and minimum scopes for vac-services internal APIs.
 *
...
const DEFAULT_CALLERS = [
    ...
    'document-presentation-service' => [
        'type' => ServiceType::API,
        'scopes' => ['vac.read'],
        'compose_secret' => 'DOCUMENT_PRESENTATION_SERVICE_REGISTRY_SECRET',
    ],
];

const TARGET_SERVICE = 'vac-services';
const ALL_SCOPES = ['vac.read', 'vac.write'];
...
if ($tenantId === null || $tenantId === '') {
    fwrite(STDERR, "Usage: php bin/seed-vac-service-s2s.php --tenant-id=<uuid> [--callers=visa-services] [--all-scopes] [--skip-credentials] [--rotate-credentials]\n");
    exit(1);
}
...
$target = ensureVacService($serviceRepository, $tenantId, TARGET_SERVICE, ServiceType::API);
...
foreach ($callerNames as $callerName) {
    $profile = resolveVacCallerProfile($callerName, $grantAllScopes);
    $caller = ensureVacService($serviceRepository, $tenantId, $callerName, $profile['type']);
...
fwrite(STDOUT, "\n--- vac-services (php-vacs) ---\n");
fwrite(STDOUT, "SERVICE_NAME=vac-services\n");
fwrite(STDOUT, "SERVICE_REGISTRY_URL=http://service-registry:80\n");
fwrite(STDOUT, "SERVICE_REGISTRY_ISSUER=service-registry\n");
fwrite(STDOUT, "# Outbound to user-service: set VACS_SERVICE_REGISTRY_SECRET via seed-user-service-s2s.php\n");
 ...
 */
function resolveVacCallerProfile(string $callerName, bool $grantAllScopes): array
{
    if ($grantAllScopes) {
        return [
            'type' => ServiceType::API,
